...
이 문제는 input 함수 대신 sys 모듈 안의 stdin.readline 함수를 사용하는 문제이다. 백준 온라인 저지에서 문제를 풀면 제출한 코드가 실행되는데 걸리는 시간을
확인할 수 있는데 이 문제에서 input 함수를 사용하면 시간 초과로 정답이 되지 않는 것을 확인할 수 있다. 

sys 모듈을 불러와서 사용 가능하다. 사용 방식과 기능은 input 함수를 사용할 때와 동일하다. 입력되는 문자는 사용자가 숫자를 입력하더라도 문자열로 입력된다. 
